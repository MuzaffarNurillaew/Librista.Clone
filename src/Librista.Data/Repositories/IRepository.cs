...
public interface IRepository
{
    ...
    Task<T> SelectAsync<T>(Expression<Func<T, bool>> expression, bool shouldTrack = true, string[]? includes = null, CancellationToken cancellationToken = default)
        where T : Auditable;
    IQueryable<T> SelectAll<T>(Expression<Func<T, bool>> expression, string[]? includes = null)
        where T : Auditable;
    
    Task<T> UpdateAsync<T>(Expression<Func<T, bool>> expression, T entity, bool shouldSave = true, CancellationToken cancellationToken = default)
        where T : Auditable;
    
    Task<T> DeleteAsync<T>(Expression<Func<T, bool>> expression, bool shouldSave = true, CancellationToken cancellationToken = default)
        where T : Auditable;
